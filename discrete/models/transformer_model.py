...
class XEyTransformerLayer(nn.Module):
    ...
    def forward(self, X: Tensor, E: Tensor, y, node_mask: Tensor):
        ...
        newX, newE, new_y = self.self_attn(X, E, y, node_mask=node_mask)  # Encoder: 1. self-Attention
...
class DigNetGraphTransformer(nn.Module):
    ...
    def forward(self, x, edge_attr, t_step, edge_index, ALLedge_index, node_mask, noisy_data):
        ...
        after_in = network_preprocess.PlaceHolder(X=self.mlp_in_X(X), E=self.mlp_in_E(E), y=self.mlp_in_y(y)).mask(node_mask) # 使用掩码矩阵更新X E Y特征（将数据处理成可变长度）
        ...
        return network_preprocess.PlaceHolder(X=X, E=E, y=y).mask(node_mask)
